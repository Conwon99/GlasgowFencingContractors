...
export const ValueCard = (props: ValueCardProps) => {
  return (
    <div
      className={`relative items-start box-border caret-transparent flex flex-col justify-start overflow-hidden p-5 rounded-r-[20px] rounded-b-[20px] ${props.variant}`}
    >
      <div className="relative bg-[#323232] box-border caret-transparent z-[2] -ml-5 -mt-5 mb-[30px] pr-2.5 pb-2.5 rounded-br-[30px]">
        <img
      ...
      <h2
        className={`text-xl font-bold box-border caret-transparent leading-6 mb-2.5 md:text-2xl md:leading-[28.8px] ${props.titleClassName || "text-white"}`}
      >
        {props.title}
      </h2>
      <p
        className={`box-border caret-transparent mb-2.5 ${props.descriptionClassName || "text-gray-200"}`}
      >
        {props.description}
      </p>
    </div>
