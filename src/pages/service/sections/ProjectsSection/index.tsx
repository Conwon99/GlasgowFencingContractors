...
export const ServiceProjectsSection = ({ category = "fencing", limit = 3 }: ServiceProjectsSectionProps) => {
  const projects = category === "fencing" 
    ? getFencingProjects().slice(0, limit)
    : category === "decking"
    ? getDeckingProjects().slice(0, limit)
    : [];
  ...
  const heading = category === "fencing" ? "Our Fencing Projects" : "Our Decking Projects";
  const viewAllLink = category === "fencing" ? "/fencing-projects" : "/decking-projects";
  const viewAllText = category === "fencing" ? "View all fencing projects" : "View all decking projects";

  return (
    <section className="bg-transparent box-border caret-transparent py-[50px] md:py-[100px]">
